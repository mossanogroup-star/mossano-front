...
async function main() {
  ...
  try {
    ...
    console.error('✗ could not load dist-ssr/entry-server.js — run "npm run build:ssr" first.')
  ...
  for (const url of urls) {
    ...
    if (!markup || markup.length < 2000) {
      console.error(`✗ ${url} rendered only ${markup?.length ?? 0} chars — aborting.`)
      process.exit(1)
    }
  ...
  if (thin) console.log(`  (${thin} are short pages — expected for application stubs)`)
}
